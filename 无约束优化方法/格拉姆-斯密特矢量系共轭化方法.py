...
from functions import fun2, fun1, fun3, fun4, fun5, fun6
...
def display(x_range, y_range, function, X0=None, delta=0.01):
    ...
    while abs((X[0, 0] - X_new[0, 0]) ** 2 + (X[1, 0] - X_new[1, 0]) ** 2) > delta:
        ...
        a = GetA(function, X, -grad)  # 计算步长，传进去的一定是方向，即负梯度
        if a is None:
    ...
    if count == 0:
        count += 1
    print("一共迭代了{:}次\n极小值点为({:.2f},{:.2f})\n极小值为{:.2f}"
          .format(count, X_new[0, 0], X_new[1, 0], function(X_new[0, 0], X_new[1, 0])))
    print(grad_list)
    print("fx = ", fx)
